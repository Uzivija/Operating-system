    ...
    public void kill(PCB process) throws Exception{
        if(process.getID()==0){
            ...
            if(possible==true){
                ...
                scheduler.check();
                if(Shell.open_file_table.findFile(process) != null)
                {
...
//            }
        ...
        if(ProcessList.contains(proc)){
            ...
            System.out.println(lev+"ID rodzica: "+parent+" | Nazwa procesu: "+proc.getName()+" ID procesu: "+proc.getID()+" | ID dzieci procesu: "+children);
