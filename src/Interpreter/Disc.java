...
public class Disc
{
        ...
        if(catalog.get(name) != null)
        {
            ...
            if (inodes_table[num] != null)
            {
                ...
                for (int i = 0; i < inodes_table[num].permissions.PermList.size(); i++)
                {
                    ...
                    if(inodes_table[num].permissions.PermList.get(i).getUserID().equals(userName) && ((inodes_table[num].permissions.PermList.get(i).getRW() == 1 || inodes_table[num].permissions.PermList.get(i).getRW() == 3)))
    ...
    public void deleteFile(String name, String userName) throws Exception
    {
        if(catalog.get(name) != null)
        {
            ...
            if(inodes_table[num] != null)
            {
                ...
                {
                    if(inodes_table[num].permissions.PermList.get(l).getUserID().equals(userName) && inodes_table[num].permissions.PermList.get(l).getOwner())
                    {
                        flag = true;
                        if (!inodes_table[num].state)
                        {
    ...
    public void deleteContent(String name)
    {
        File file = catalog.get(name);
        int num = file.number;
        int amount = inodes_table[num].size > 32 ? 2 : 1;
        int direct, inDirect;
        ...
        else if(amount == 2)
        {
            ...
            Arrays.fill(disc, direct*blockSize, direct*blockSize + blockSize, (char)-1);
            inDirect = inodes_table[num].blocks[1];
            inodes_table[num].blocks[1] = -1;
            while((int)disc[inDirect*32+tmp] != 65535)
            {
                int f = (int)disc[inDirect*32+tmp];
                Arrays.fill(disc, f*blockSize, f*blockSize+blockSize, (char)0);
                bitArray.set(f);
                ++freeBlockAmount;
                tmp++;
            }
            Arrays.fill(disc, inDirect*blockSize, inDirect*blockSize + blockSize, (char)0);
            bitArray.set(inDirect);
            ++freeBlockAmount;

            IndexBlocks.remove(IndexBlocks.indexOf(inDirect));
        }
        inodes_table[num].size = 0;
    }

    public void printBitArray()
    {
        System.out.println("||||||||||||| TABLICA BITOW |||||||||||||");
        for(int i = 0; i < 32; i++)
        {
            if(bitArray.get(i))
            {
                System.out.println(i + ": WOLNY");
            }
            else
            {
                System.out.println(i + ": ZAJETY");
            }
        }
        System.out.println("|||||||||||||||||||||||||||||||||||||||||\n");
    }
